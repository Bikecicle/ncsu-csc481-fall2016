...
public class EventManager implements Serializable {
	...
	public void deregisterObject(int guid) {
		synchronized (registrar) {
			for (int i = 0; i < EConstant.TOTAL_EVENT_TYPES; i++) {
				List<EventHandler> handlers = registrar.get(i);
				if (handlers != null) {
					synchronized (handlers) {
						handlers.removeIf(new Predicate<EventHandler>() {

							@Override
							public boolean test(EventHandler t) {
